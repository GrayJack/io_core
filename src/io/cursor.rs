...
use crate::io::{
    self, BorrowedCursor, BufRead, ErrorKind, IoSlice, IoSliceMut, Read, SeekFrom, Write,
};
...
fn slice_write_vectored(
    pos_mut: &mut u64, slice: &mut [u8], bufs: &[IoSlice<'_>],
) -> io::Result<usize> {
    let mut nwritten = 0;
    for buf in bufs {
...
fn vec_write_all_vectored<A>(
    pos_mut: &mut u64, vec: &mut Vec<u8, A>, bufs: &[IoSlice<'_>],
) -> io::Result<usize>
where
    A: Allocator,
{
...
/// before writing anything from that point
#[cfg(all(feature = "alloc", not(feature = "nightly")))]
fn vec_write_all(pos_mut: &mut u64, vec: &mut Vec<u8>, buf: &[u8]) -> io::Result<usize> {
    let buf_len = buf.len();
    let mut pos = reserve_and_pad(pos_mut, vec, buf_len)?;
...
fn vec_write_all_vectored(
    pos_mut: &mut u64, vec: &mut Vec<u8>, bufs: &[IoSlice<'_>],
) -> io::Result<usize> {
    // For safety reasons, we don't want this sum to overflow ever.
    // If this saturates, the reserve should panic to avoid any unsound writing.
    let buf_len = bufs.iter().fold(0usize, |a, b| a.saturating_add(b.len()));
