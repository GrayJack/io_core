...
mod error;
pub use error::{Error, ErrorKind, RawOsError, Result, SimpleMessage};

mod borrowed_buf;
pub use borrowed_buf::{BorrowedBuf, BorrowedCursor, TrackingBorrowedBuf, TrackingBorrowedCursor};
...
//   overhead, all reads incur costs proportional to buffer size (#110650)
//
#[cfg(feature = "alloc")]
pub(crate) fn default_read_to_end<R: Read + ?Sized>(
    r: &mut R, buf: &mut Vec<u8>, size_hint: Option<usize>,
) -> Result<usize> {
...
pub(crate) fn default_read_to_string<R: Read + ?Sized>(
    r: &mut R, buf: &mut String, size_hint: Option<usize>,
) -> Result<usize> {
...
pub(crate) fn default_read_buf_exact<R: Read + ?Sized>(
    this: &mut R, mut cursor: BorrowedCursor<'_, u8>,
) -> Result<()> {
...
pub(crate) fn default_write_fmt<W: Write + ?Sized>(
    this: &mut W, args: fmt::Arguments<'_>,
) -> Result<()> {
...
pub trait Read {
    ...
    /// # Usage Notes
    ///
    /// `read_to_string` attempts to read a source until EOF, but many sources are continuous
    /// streams that do not send EOF. In these cases, `read_to_string` will block indefinitely.
    /// Standard input is one such stream which may be finite if piped, but is typically
    /// continuous. For example, `cat file | my-rust-program` will correctly terminate with an
    /// `EOF` upon closure of cat. Reading user input or running programs that remain open
    /// indefinitely will never terminate the stream with `EOF` (e.g. `yes | my-rust-program`).
    ///
    /// Using `.lines()` with a [`BufReader`] or using [`read`] can provide a better solution
    ///
    /// [`read`]: Read::read
...
impl<T: Seek> Seek for Take<T> {
    ...
    fn seek_relative(&mut self, offset: i64) -> Result<()> {
        if self.position().checked_add_signed(offset).is_none_or(|p| p > self.len) {
...
impl<R: Read> Iterator for Bytes<R> {
    ...
    #[cfg(not(feature = "nightly"))]
    fn size_hint(&self) -> (usize, Option<usize>) {
        // impossible to do this without specialization, too instable to use out of libstd
        // SizeHint::size_hint(&self.inner)
        (0, None)
    }
}
