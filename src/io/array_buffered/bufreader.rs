...
///     let mut reader: ArrayBufReader<_, 512> = ArrayBufReader::new(f);
///
...
impl<R: Read, const N: usize> ArrayBufReader<R, N> {
    ...
    /// use std::fs::File;
    ///
    /// use io_core::io::{self, ArrayBufReader};
    ///
    /// fn main() -> io::Result<()> {
    ///     let f = File::open("log.txt")?;
    ///     let reader = ArrayBufReader::<_, 512>::new(f);
    ///     Ok(())
    /// }
    /// ```
...
impl<R: ?Sized, const N: usize> ArrayBufReader<R, N> {
    ...
    /// use std::fs::File;
    ///
    /// use io_core::io::{self, ArrayBufReader, BufRead};
    ///
    /// fn main() -> io::Result<()> {
    ///     let f = File::open("log.txt")?;
...
impl<R: ?Sized + Read, const N: usize> Read for ArrayBufReader<R, N> {
    ...
    fn read_to_string(&mut self, buf: &mut String) -> io::Result<usize> {
        ...
        if buf.is_empty() {
            ...
            let string = core::str::from_utf8(&bytes).map_err(|_| io::Error::INVALID_UTF8)?;
            *buf += string;
            Ok(string.len())
        }
    }
...
impl<R: ?Sized + Seek, const N: usize> Seek for ArrayBufReader<R, N> {
    ...
    ///
    /// To seek without discarding the internal buffer, use [`ArrayBufReader::seek_relative`].
    ///
