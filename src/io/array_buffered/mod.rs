...
pub use self::{
    bufreader::ArrayBufReader,
    bufwriter::{ArrayBufWriter, ArrayWriterPanicked},
    linewriter::ArrayLineWriter,
...
impl<W> IntoInnerError<W> {
    ...
    pub(super) fn new(writer: W, error: Error) -> Self {
        Self(writer, error)
    }

    /// Helper to construct a new IntoInnerError; intended to help with
    /// adapters that wrap other adapters
    pub(super) fn new_wrapped<W2>(self, f: impl FnOnce(W) -> W2) -> IntoInnerError<W2> {
